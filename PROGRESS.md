...
- **Server** (`server/` — one Cloudflare Worker + R2):
  ...
  - Default homepage is the **gallery** (links to all 18 landings); regenerate with `npm run demo`,
...
- **18 landing-page designs** (`landings/`): **10 shadcn.io DESIGN.md systems** (Linear, Vercel,
  Stripe, Supabase, Raycast, GitHub, Cursor, Notion, Figma, Hugging Face — each built from the
  system's real fetched tokens) + **8 originals** (Midnight, Scanini, Shellshare, PostHog,
  Claude, Funkadelic, Groovy Hacker, Pixel Funk). A **gallery** (`gallery/`) links to all 18 and is the homepage. Every page (and the
...
- **Reproducible demo** — `npm run demo` (`scripts/publish-demo.mjs`) redeploys all 18 landings +
